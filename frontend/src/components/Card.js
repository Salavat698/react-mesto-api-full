...
function Card (props){
  ...
  const isOwn = props.dataCards.owner === userId._id;
// Создаём переменную, которую после зададим в `className` для кнопки удаления
  const cardDeleteButtonClassName = (
    `element__backet ${isOwn ? 'element__backet_active' : ' element__backet_hidden'}`
  );

//   const isLiked = props.dataCards.likes.some(i => i === userId._id);
// console.log( isLiked)
    // Определяем, есть ли у карточки лайк, поставленный текущим пользователем
    // console.log(props.dataCards.likes)
  ...
  // console.log(isLiked)
  // Создаём переменную, которую после зададим в `className` для кнопки лайка перекрашиваем
